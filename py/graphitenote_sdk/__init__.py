...
from graphitenote_sdk.utility import register

# Load features
from graphitenote_sdk.feature.base_feature import GraphiteNoteBaseFeature
from graphitenote_sdk.features import _has_feature, _make_feature
...
class GraphiteNoteSDK:
    ...
    def get_utility(self):
        return GraphiteNoteUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("GraphiteNoteSDK: graphql: " + str(msg))
            res["graphql"] = errors
    ...
    def DatasetFill(self, data=None) -> "DatasetFillEntity":
        ...
        return DatasetFillEntity(self, data)


    def ModelInfo(self, data=None) -> "ModelInfoEntity":
        """Entity factory: client.ModelInfo().list() / client.ModelInfo().load({"id": ...})."""
        from graphitenote_sdk.entity.model_info_entity import ModelInfoEntity
        return ModelInfoEntity(self, data)


    def ModelResult(self, data=None) -> "ModelResultEntity":
        """Entity factory: client.ModelResult().list() / client.ModelResult().load({"id": ...})."""
        from graphitenote_sdk.entity.model_result_entity import ModelResultEntity
        return ModelResultEntity(self, data)


    def Prediction(self, data=None) -> "PredictionEntity":
        """Entity factory: client.Prediction().list() / client.Prediction().load({"id": ...})."""
        from graphitenote_sdk.entity.prediction_entity import PredictionEntity
        return PredictionEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "GraphiteNoteSDK":
        if sdkopts is None:
            sdkopts = {}
