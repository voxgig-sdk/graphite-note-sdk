    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GRAPHITENOTE_TEST_DATASET_FILL_ENTID JSON to run live")
        client = setup["client"]
        ...
        dataset_fill_ref01_data = helpers.to_map(dataset_fill_ref01_ent.create(dataset_fill_ref01_data, None))
...
def _dataset_fill_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "GRAPHITENOTE_TEST_DATASET_FILL_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GRAPHITENOTE_TEST_DATASET_FILL_ENTID"))
