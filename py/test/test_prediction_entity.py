    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GRAPHITE_NOTE_TEST_PREDICTION_ENTID JSON to run live")
        client = setup["client"]
        ...
        prediction_ref01_data = helpers.to_map(runner.entity_data(prediction_ref01_ent.create(prediction_ref01_data, None)))
...
def _prediction_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "GRAPHITE_NOTE_TEST_PREDICTION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GRAPHITE_NOTE_TEST_PREDICTION_ENTID"))
