...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.GRAPHITENOTE_TEST_LIVE ||
    'TRUE' === process.env.GRAPHITENOTE_TEST_OVERRIDE
  ) {
  ...
  m.GRAPHITENOTE_TEST_EXPLAIN = process.env.GRAPHITENOTE_TEST_EXPLAIN || m.GRAPHITENOTE_TEST_EXPLAIN
