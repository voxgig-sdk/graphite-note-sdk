...
const __1 = require("..");
const SDK_NAME = 'GraphiteNoteSDK';
...
const TEST_SEED = { "entity": { "dataset": { "test01": { "id": "test01" } }, "dataset_fill": { "test01": { "id": "test01" } }, "model_info": { "test01": { "id": "test01" } }, "model_result": { "test01": { "id": "test01" } }, "prediction": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+GraphiteNoteSDK\b/.test(code) ||
        /\bGraphiteNoteSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+GraphiteNoteSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/GraphiteNoteSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
