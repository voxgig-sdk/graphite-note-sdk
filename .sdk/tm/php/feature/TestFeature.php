...
class GraphiteNoteTestFeature extends GraphiteNoteBaseFeature
{
    ...
    public function init(GraphiteNoteContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (GraphiteNoteContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = GraphiteNoteParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (GraphiteNoteContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(GraphiteNoteContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = GraphiteNoteParam::call($ctx, $k);
