...
// GraphiteNote SDK retry feature
...
class GraphiteNoteRetryFeature extends GraphiteNoteBaseFeature
{
    ...
    public function init(GraphiteNoteContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (GraphiteNoteContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(GraphiteNoteContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(GraphiteNoteContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
