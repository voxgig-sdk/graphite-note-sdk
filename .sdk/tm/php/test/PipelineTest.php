...
// GraphiteNote SDK pipeline test
//
...
{
    private static function utility(): GraphiteNoteUtility
    {
        return new GraphiteNoteUtility();
    }
    ...
    private static function ctx(array $over = []): GraphiteNoteContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GraphiteNoteContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GraphiteNoteOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GraphiteNoteError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GraphiteNoteOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GraphiteNoteMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GraphiteNoteOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GraphiteNoteMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GraphiteNoteOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GraphiteNoteMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GraphiteNoteSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GraphiteNoteMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GraphiteNoteResponse([]),
            'result' => new GraphiteNoteResult([]),
        ]);
        [, $err] = GraphiteNoteMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new GraphiteNoteSpec([]),
            'response' => null,
            'result' => new GraphiteNoteResult([]),
        ]);
        [, $err] = GraphiteNoteMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GraphiteNoteSpec(['step' => 's']),
            'response' => new GraphiteNoteResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GraphiteNoteResult([]),
        ]);
        [, $err] = GraphiteNoteMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GraphiteNoteSpec(['step' => 's']),
            'response' => new GraphiteNoteResponse(self::resp(200, ['v' => 1])),
            'result' => new GraphiteNoteResult([]),
        ]);
        [, $err] = GraphiteNoteMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GraphiteNoteSpec(['step' => 's']),
            'response' => new GraphiteNoteResponse(self::resp(200, ['v' => 2])),
            'result' => new GraphiteNoteResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GraphiteNoteMakeResponse::call($ctx);
    ...
    {
        $preset = new GraphiteNoteResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GraphiteNoteSpec([]),
            'response' => new GraphiteNoteResponse([]),
            'result' => new GraphiteNoteResult([]),
        ...
        [$got, $err] = GraphiteNoteMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GraphiteNoteResult([])]);
        [, $err] = GraphiteNoteMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GraphiteNoteSpec([]), 'result' => null]);
        [, $err] = GraphiteNoteMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GraphiteNoteSpec(['step' => 's']),
            'result' => new GraphiteNoteResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GraphiteNoteOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GraphiteNoteMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GraphiteNoteSpec(['step' => 's']),
            'result' => new GraphiteNoteResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GraphiteNoteOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GraphiteNoteMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GraphiteNoteResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GraphiteNoteSpec([]),
            'result' => new GraphiteNoteResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GraphiteNoteMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GraphiteNoteMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GraphiteNoteError('boom', 'boom');
        $utility->fetcher = function (GraphiteNoteContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GraphiteNoteSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GraphiteNoteMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GraphiteNoteContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GraphiteNoteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GraphiteNoteResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        GraphiteNoteMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GraphiteNoteContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GraphiteNoteSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GraphiteNoteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GraphiteNoteResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GraphiteNoteSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GraphiteNoteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GraphiteNoteMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GraphiteNoteMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GraphiteNoteResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GraphiteNoteDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GraphiteNoteResult(['ok' => false])]);
        $this->expectException(GraphiteNoteError::class);
        GraphiteNoteDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GraphiteNoteResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GraphiteNoteMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        GraphiteNoteMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GraphiteNoteBaseFeature();
        $b = new GraphiteNoteBaseFeature();
        GraphiteNoteFeatureAdd::call($ctx, $a);
        GraphiteNoteFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GraphiteNoteBaseFeature
    {
        $f = new GraphiteNoteBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GraphiteNoteFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GraphiteNoteContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GraphiteNoteSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GraphiteNotePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        GraphiteNotePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GraphiteNoteResponse(['headers' => null]),
            'result' => new GraphiteNoteResult([]),
        ]);
        GraphiteNoteResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        GraphiteNoteResultBody::call($ctx);
