...
class FtCtrl extends GraphiteNoteControl
{
...
class FtClient
{
    ...
    private GraphiteNoteUtility $utility;

    public function __construct(GraphiteNoteUtility $utility, array $options)
    {
    ...
    public function get_utility(): GraphiteNoteUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GraphiteNoteContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GraphiteNoteContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GraphiteNoteContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GraphiteNoteConfig::make_feature($name);
    ...
    private static function buildUrl(GraphiteNoteSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GraphiteNoteError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GraphiteNoteResponse($fetched);
        }
    ...
    private function fail(GraphiteNoteContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GraphiteNoteContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
