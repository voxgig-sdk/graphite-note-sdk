...
from projectname_sdk import GraphiteNoteSDK
from projectname_sdk.core.spec import GraphiteNoteSpec
from projectname_sdk.core.result import GraphiteNoteResult
from projectname_sdk.core.response import GraphiteNoteResponse
from projectname_sdk.core.operation import GraphiteNoteOperation
from projectname_sdk.core.error import GraphiteNoteError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import GraphiteNoteBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return GraphiteNoteError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_exists(self):
        client = GraphiteNoteSDK.test(None, None)
    ...
    def test_feature_init_basic(self):
        init_client = GraphiteNoteSDK.test(None, None)
        ...
        class TestInitFeature(GraphiteNoteBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(GraphiteNoteBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = GraphiteNoteSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = GraphiteNoteSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = GraphiteNoteSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = GraphiteNoteOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = GraphiteNoteSDK.test(None, setup_opts)
