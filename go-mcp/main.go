// Command GraphiteNote-mcp is an MCP server exposing the GraphiteNote
// SDK as MCP tools.
//
// Two tools are registered:
//
//	GraphiteNote_list  — list records of an entity
//	GraphiteNote_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: GRAPHITE_NOTE_APIKEY carries the API key and
	// GRAPHITE_NOTE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GRAPHITE_NOTE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GRAPHITE_NOTE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGraphiteNoteSDK(opts)
