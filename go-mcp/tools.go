...
type Args struct {
	Entity string         `json:"entity" jsonschema:"dataset | dataset_fill | model_info | model_result | prediction"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.GraphiteNoteSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "graphite-note_list",
		Description: "List records from GraphiteNote. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "graphite-note_load",
		Description: "Load a single record from GraphiteNote. " +
...
func runOp(client *sdk.GraphiteNoteSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.GraphiteNoteSDK, name string) (sdk.GraphiteNoteEntity, error) {
	switch strings.ToLower(name) {
	case "dataset":
		return client.Dataset(nil), nil
	case "dataset_fill":
		return client.DatasetFill(nil), nil
	case "model_info":
		return client.ModelInfo(nil), nil
	case "model_result":
		return client.ModelResult(nil), nil
	case "prediction":
		return client.Prediction(nil), nil
