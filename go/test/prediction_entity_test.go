...
func TestPredictionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GRAPHITENOTE_TEST_PREDICTION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		predictionRef01Data = core.ToMapAny(predictionRef01DataResult)
...
func predictionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GRAPHITENOTE_TEST_PREDICTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GRAPHITENOTE_TEST_PREDICTION_ENTID"])
