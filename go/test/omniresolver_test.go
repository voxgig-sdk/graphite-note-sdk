...
import (
	...
	omni "github.com/voxgig-sdk/graphite-note-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GraphiteNoteError); is {
				out := map[string]any{
					"name":    "GraphiteNoteError",
...
func omniCtx(arg any, client *sdk.GraphiteNoteSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GraphiteNoteSDK); is {
			client = live
