...
func TestDatasetFillEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GRAPHITENOTE_TEST_DATASET_FILL_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		datasetFillRef01Data = core.ToMapAny(datasetFillRef01DataResult)
...
func dataset_fillBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GRAPHITENOTE_TEST_DATASET_FILL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GRAPHITENOTE_TEST_DATASET_FILL_ENTID"])
