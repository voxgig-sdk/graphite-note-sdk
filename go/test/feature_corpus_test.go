...
//
// NewGraphiteNoteSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.GraphiteNoteSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewGraphiteNoteSDK(opts)
}
...
func fcCandidates(client *sdk.GraphiteNoteSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.GraphiteNoteSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.GraphiteNoteSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.GraphiteNoteError
