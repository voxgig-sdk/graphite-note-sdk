...
func TestModelInfoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		modelInfoRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.model_info")))
...
func model_infoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GRAPHITE_NOTE_TEST_MODEL_INFO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GRAPHITE_NOTE_TEST_MODEL_INFO_ENTID"])
