...
PredictionEntity.__index = PredictionEntity


function PredictionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PredictionEntity)
  self._name = "prediction"
  self._client = client
...
function PredictionEntity:get_name()
  return self._name
end


function PredictionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PredictionEntity.new(self._client, opts)
end
...
function PredictionEntity:mark_deleted()
  self._deleted = true
end


function PredictionEntity:deleted()
  return true == self._deleted
end


function PredictionEntity:data_set(args)
  if args ~= nil then
...
function PredictionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PredictionEntity:match_set(args)
  if args ~= nil then
...
function PredictionEntity:match_get()
...
function PredictionEntity:stream(action, args, callopts)
...
---@param reqdata PredictionCreateData
---@param ctrl? table
---@return Prediction
---@return string? err
function PredictionEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function PredictionEntity:_run_op(ctx, post_done)
  local utility = self._utility
