...
local ModelInfoEntity = {}
ModelInfoEntity.__index = ModelInfoEntity


function ModelInfoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ModelInfoEntity)
  self._name = "model_info"
  self._client = client
...
function ModelInfoEntity:get_name()
  return self._name
end


function ModelInfoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ModelInfoEntity.new(self._client, opts)
end
...
function ModelInfoEntity:mark_deleted()
  self._deleted = true
end


function ModelInfoEntity:deleted()
  return true == self._deleted
end


function ModelInfoEntity:data_set(args)
  if args ~= nil then
...
function ModelInfoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ModelInfoEntity:match_set(args)
  if args ~= nil then
...
function ModelInfoEntity:match_get()
...
function ModelInfoEntity:stream(action, args, callopts)
...
---@param reqmatch ModelInfoLoadMatch
---@param ctrl? table
---@return ModelInfo
---@return string? err
function ModelInfoEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ModelInfoEntity:_run_op(ctx, post_done)
  local utility = self._utility
