...
local DatasetFillEntity = {}
DatasetFillEntity.__index = DatasetFillEntity


function DatasetFillEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DatasetFillEntity)
  self._name = "dataset_fill"
  self._client = client
...
function DatasetFillEntity:get_name()
  return self._name
end


function DatasetFillEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DatasetFillEntity.new(self._client, opts)
end
...
function DatasetFillEntity:mark_deleted()
  self._deleted = true
end


function DatasetFillEntity:deleted()
  return true == self._deleted
end


function DatasetFillEntity:data_set(args)
  if args ~= nil then
...
function DatasetFillEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DatasetFillEntity:match_set(args)
  if args ~= nil then
...
function DatasetFillEntity:match_get()
...
function DatasetFillEntity:stream(action, args, callopts)
...
---@param reqdata DatasetFillCreateData
---@param ctrl? table
---@return DatasetFill
---@return string? err
function DatasetFillEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function DatasetFillEntity:_run_op(ctx, post_done)
  local utility = self._utility
