...
local ModelResultEntity = {}
ModelResultEntity.__index = ModelResultEntity


function ModelResultEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ModelResultEntity)
  self._name = "model_result"
  self._client = client
...
function ModelResultEntity:get_name()
  return self._name
end


function ModelResultEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ModelResultEntity.new(self._client, opts)
end
...
function ModelResultEntity:mark_deleted()
  self._deleted = true
end


function ModelResultEntity:deleted()
  return true == self._deleted
end


function ModelResultEntity:data_set(args)
  if args ~= nil then
...
function ModelResultEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ModelResultEntity:match_set(args)
  if args ~= nil then
...
function ModelResultEntity:match_get()
...
function ModelResultEntity:stream(action, args, callopts)
...
---@param reqdata ModelResultCreateData
---@param ctrl? table
---@return ModelResult
---@return string? err
function ModelResultEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ModelResultEntity:_run_op(ctx, post_done)
  local utility = self._utility
