...
describe("DatasetEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GRAPHITE_NOTE_TEST_DATASET_ENTID JSON to run live")
      return
    end
    ...
    local dataset_ref01_data = helpers.to_map(vs.getprop(
...
function dataset_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("GRAPHITE_NOTE_TEST_DATASET_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["GRAPHITE_NOTE_TEST_DATASET_ENTID"])
