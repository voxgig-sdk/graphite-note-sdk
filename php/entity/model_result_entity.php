...
// GraphiteNote SDK ModelResult entity
...
class ModelResultEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ModelResultEntity($this->_client, $opts);
    }

    /**
     * @param ModelResult|array $args ModelResult data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = GraphiteNoteHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ModelResult|array The current ModelResult data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ModelResult fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = GraphiteNoteHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ModelResult fields).
     */
    ...
    /**
     * Create a new ModelResult.
     *
     * @param ModelResultCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ModelResultCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ModelResult|array The created ModelResult as an assoc-array at the
     *   SDK boundary; throws GraphiteNoteError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = GraphiteNoteHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
