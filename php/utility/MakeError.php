...
class GraphiteNoteMakeError
{
    public static function call(?GraphiteNoteContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new GraphiteNoteContext([], null);
        }
        $op = $ctx->op ?? new GraphiteNoteOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new GraphiteNoteResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof GraphiteNoteError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new GraphiteNoteError('', $msg, $ctx);
        ...
        if ($err instanceof GraphiteNoteError) {
