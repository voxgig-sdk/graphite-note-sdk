...
class DatasetFillEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GRAPHITE_NOTE_TEST_DATASET_FILL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $dataset_fill_ref01_data = Helpers::to_map(is_object($dataset_fill_ref01_data_result) && method_exists($dataset_fill_ref01_data_result, 'data_get') ? $dataset_fill_ref01_data_result->data_get() : $dataset_fill_ref01_data_result);
...
function dataset_fill_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("GRAPHITE_NOTE_TEST_DATASET_FILL_ENTID");
