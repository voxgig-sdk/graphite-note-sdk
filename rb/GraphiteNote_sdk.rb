...
require_relative 'GraphiteNote_types'
...
class GraphiteNoteSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GraphiteNoteHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GraphiteNoteHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GraphiteNoteFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    GraphiteNoteUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GraphiteNoteHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = GraphiteNoteHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GraphiteNoteError.new(
        "graphql_error", "GraphiteNoteSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def DatasetFill(data = nil)
    ...
    DatasetFillEntity.new(self, data)
  end


  # Canonical facade: client.ModelInfo.list / client.ModelInfo.load({ "id" => ... })
  def ModelInfo(data = nil)
    require_relative 'entity/model_info_entity'
    ModelInfoEntity.new(self, data)
  end


  # Canonical facade: client.ModelResult.list / client.ModelResult.load({ "id" => ... })
  def ModelResult(data = nil)
    require_relative 'entity/model_result_entity'
    ModelResultEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = GraphiteNoteSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
