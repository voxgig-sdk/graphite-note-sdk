...
class ModelInfoEntity
  ...
  def make
    opts = @_entopts.dup
    ModelInfoEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = GraphiteNoteHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ModelInfo, Hash] the current ModelInfo data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = GraphiteNoteHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ModelInfo fields)
  def match_get
  ...
  #
  # @param reqmatch [ModelInfoLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ModelInfo.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ModelInfo, Hash] the loaded ModelInfo; raises GraphiteNoteError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GraphiteNoteHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
