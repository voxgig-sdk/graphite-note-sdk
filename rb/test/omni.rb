...
module GraphiteNoteOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if GraphiteNoteOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = GraphiteNoteOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| GraphiteNoteOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || GraphiteNoteOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = GraphiteNoteSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = GraphiteNoteError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { GraphiteNoteOmni.errify(err) }
