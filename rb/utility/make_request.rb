...
module GraphiteNoteUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GraphiteNoteResponse.new({})
    ...
    if fetch_err
      ...
      response = GraphiteNoteResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GraphiteNoteResponse.new(fetched)
    else
