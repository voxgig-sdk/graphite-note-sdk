...
boru-driven command-line client **and** interactive REPL for the GraphiteNote
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/graphite-note-cli)
make build

# 2. See usage (words, entities, env vars)
./graphite-note-cli --help
...
GRAPHITE_NOTE_BASE=https://api.example.com ./graphite-note-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/graphite-note-cli
   ```
...
Run with no arguments to open a REPL (prompt `graphite-note>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/graphite-note-cli`. |
...
(`./graphite-note-cli <expr>`) and interactively (the REPL), and expressions compose the
